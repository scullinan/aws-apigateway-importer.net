    ...
    public class VendorExtensionsConverter : JsonConverter
    {
        ...
        public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
        {
            ...
            foreach (var jsonProp in jsonContract.Properties)
            {
                ...
                else if (jsonProp.PropertyName == "definitions")
                {
                    writer.WritePropertyName(jsonProp.PropertyName);
                    writer.WriteStartObject();

                    var defintions = (IDictionary<string, Schema>)propValue;
                    if (defintions.Any())
                    {
                        foreach (var entry in defintions)
                        {
                            writer.WritePropertyName(entry.Key);
                            serializer.Serialize(writer, entry.Value);
                        }
                ...
                {
                    if (
                        !jsonProp.AttributeProvider.GetAttributes(typeof (AwsApiGatewayJsonIgnoreAttribute), false)
                            .Any())
                    {
                        writer.WritePropertyName(jsonProp.PropertyName);
